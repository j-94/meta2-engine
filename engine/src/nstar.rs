...
pub async fn nstar_hud_handler() -> impl IntoResponse {
    ...
    if let Ok(s) = fs::read_to_string(&path).await {
        for line in s.lines().rev().take(100) {
            rows.push(html_escape_min(line));
        }
    }
    let body = format!(
        r#"<!doctype html><html><head><meta charset='utf-8'><title>N* HUD</title>
    ...
    </head><body><h1>N* Receipts (tail)</h1><p>File: {}</p><pre>{}</pre></body></html>"#,
        path,
        rows.join("\n")
    );
    Html(body)
