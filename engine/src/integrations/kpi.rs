...
pub async fn weekly_planning() -> anyhow::Result<Vec<AgentGoal>> {
    ...
    emit_telemetry("kpi", "weekly_planning", None, json!({
...
pub async fn track_kpi_impact(goal: &AgentGoal, actual_impact: f32) -> anyhow::Result<()> {
    ...
    emit_telemetry("kpi", "impact_tracking", None, json!({
        "goal_id": goal.id,
    ...
    tracing::info!("KPI impact for {}: estimated {:.2}, actual {:.2}", 
                   goal.id, goal.estimated_impact, actual_impact);
    Ok(())
}

async fn emit_telemetry(component: &str, event_type: &str, run_id: Option<String>, metadata: serde_json::Value) {
