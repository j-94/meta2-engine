...
pub async fn create_pr_if_confident(manifest: &Manifest, bits: &Bits) -> anyhow::Result<Option<PullRequest>> {
    // Gate: Only create PR if confidence is high
    if bits.t < 0.8 || bits.e > 0.0 {
        emit_telemetry("monorepo", "pr_rejected", Some(manifest.run_id.clone()), Some(bits.clone()), json!({
    ...
    emit_telemetry("monorepo", "pr_created", Some(manifest.run_id.clone()), Some(bits.clone()), json!({
        "pr_id": pr.id,
...
pub async fn ci_gate_check(pr: &PullRequest) -> anyhow::Result<bool> {
    ...
    emit_telemetry("monorepo", "ci_check", Some(pr.run_id.clone()), None, json!({
        "pr_id": pr.id,
        "passed": passed,
        "confidence": pr.confidence
    })).await;
